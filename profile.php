													...
													while($row = $result1->fetch_assoc()) {
														...
														if ($result2->num_rows > 0) {
															...
															//echo "<img src=".$row["image"]." alt="Avatar" style="width:150px">";
																...
																/*echo '<img src="'.$row["image"].'" alt="Avatar" style="width:100px" 
																style="float:left;"/><span> '.$row["username"].'</span>';
												...
												<h3>Top 10 players past week</h3>
													<table>
														...
														<?php
															...
															$sql = "SELECT ROW_NUMBER() OVER (order by score DESC) AS rank ,id, username, score FROM users 
															...
															// output data of each row
																while($row = $result->fetch_assoc()) {
																	...
																	<td>". $row["score"]. "</td>
																	<td> <a href='follow.php?receiver_id=$row[id]&username=$row[username]'>
																	<input type='submit' id=myDIV onclick=myFunction() value='$x' class='btn'></td>
																	</tr>";
																}
																echo "</table>";
																} 
													...
													<div id="home" class="tab-pane fade in active">
														<h3>Against AI bot</h3>
															...
															<?php
																...
																$sql1 = "SELECT winner_name, loser_name from matches 
																where winner_name = '$username' OR loser_name = '$username'  ";
																$result1 = $conn->query($sql1);
																$counter = 0;
																...
																if ($result1->num_rows > 0) {
																	...
																	while($row = $result1->fetch_assoc()) {
																		$counter++;
																		echo "<tr><td >".$counter."</td><td>".$row["winner_name"]."</td><td>"
																		.$row["loser_name"]."</td><td>".$row["winner_name"]."</td></tr>";
																	}
																	echo "<hr>";
																}
																else { 
